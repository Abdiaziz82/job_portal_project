...
export default function EditDuties() {
  ...
  const [formData, setFormData] = useState(location.state?.duty || {});
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.error("Failed to update duty:", error.error);
        alert(`Failed to update duty: ${error.error}`);
      }
    } catch (error) {
      console.error("Error updating duty:", error);
      alert("An error occurred while updating duty. Please try again.");
    }
  };
  ...
  return (
    <div className="p-6 bg-white rounded-lg shadow-md w-full">
      ...
      <h2 className="text-lg font-semibold text-gray-800 mb-4">Edit Duties</h2>
