...
export default function EditPublications() {
  ...
  const [formData, setFormData] = useState(location.state?.publication || {});
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        `http://127.0.0.1:5000/publications/${formData.id}`,
        {
          method: "PUT",
      ...
      if (response.ok) {
        ...
        console.error("Failed to update publication:", error.error);
        alert(`Failed to update publication: ${error.error}`);
      }
    } catch (error) {
      console.error("Error updating publication:", error);
      alert("An error occurred while updating publication. Please try again.");
    }
  };
